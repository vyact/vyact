...
export const api = {
    ...
    async resolveToolApproval(approvalId: string, approved: boolean): Promise<void> { const response = await fetch(`${API_BASE}/tool-approvals/${encodeURIComponent(approvalId)}`, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({approved})}); await assertOk(response, 'Unable to resolve tool approval.'); },

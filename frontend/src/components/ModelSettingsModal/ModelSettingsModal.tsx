            ...
            {!profile ? <div className="model-settings-loading">{error || t('modelSettings.loading')}</div> : <div className="model-settings-body" hidden={benchmarkOpen}>
                ...
                {!tokenBudgetIsValid && limits && <div className="model-settings-budget-warning" role="alert"><p className="model-settings-error">{tokenBudgetStatus && (
                    tokenBudgetStatus.outputExcess > 0 && tokenBudgetStatus.excess > tokenBudgetStatus.outputExcess
                        ...
                        ? t('modelSettings.outputBudgetOverage', {limit: tokenNumber(tokenBudgetStatus.outputLimit), current: tokenNumber(tokenBudgetStatus.output), excess: tokenNumber(tokenBudgetStatus.outputExcess)})
                        : tokenBudgetStatus.excess > 0
                            ? t('modelSettings.tokenBudgetOverage', {limit: tokenNumber(tokenBudgetStatus.limit), current: tokenNumber(tokenBudgetStatus.total), excess: tokenNumber(tokenBudgetStatus.excess)})
                            : t('modelSettings.tokenBudgetExceeded', {total: tokenNumber(limits.tokenBudget), output: tokenNumber(limits.outputMax)})
                )}</p></div>}

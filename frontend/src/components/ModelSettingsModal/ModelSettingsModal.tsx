    ...
    return <ModalOverlay className="model-settings-overlay" onClose={benchmarkBusy ? undefined : onClose} closeOnBackdrop={false}>
        <form ref={formRef} className="model-settings-modal" onInvalidCapture={() => setAdvancedOpen(true)} onSubmit={event => {event.preventDefault(); void apply();}}>
            ...
            {!profile ? <div className="model-settings-loading">{error || t('modelSettings.loading')}</div> : <div className="model-settings-body" hidden={benchmarkOpen}>
                ...
                <label><SettingLabel label={t('modelSettings.maxOutput')} help={<ModelSettingsHelp title={t('modelSettings.maxOutput')} description={t('modelSettings.maxOutputTooltip')}/>}/><input className="model-settings-input" type="number" required min="1" max={MODEL_SETTING_INPUT_MAX.tokens} value={numberInputs.max_output_tokens ?? profile.max_output_tokens} onChange={e => updateNumber('max_output_tokens', e.target.value)}/></label>
                ...
                <label><SettingLabel label={t('modelSettings.temperature')} help={<ModelSettingsHelp title={t('modelSettings.temperature')} description={t('modelSettings.temperatureTooltip')}/>}/><input className="model-settings-input" type="number" required min="0" max={MODEL_SETTING_INPUT_MAX.temperature} step="any" value={numberInputs.temperature ?? profile.temperature} onChange={e => updateNumber('temperature', e.target.value)}/></label>
                <label><SettingLabel label={t('modelSettings.topK')} help={<ModelSettingsHelp title={t('modelSettings.topK')} description={t('modelSettings.topKTooltip')}/>}/><input className="model-settings-input" type="number" min="0" max={MODEL_SETTING_INPUT_MAX.topK} value={numberInputs.top_k ?? profile.top_k ?? ''} placeholder={t('modelSettings.modelDefault')} onChange={e => updateNumber('top_k', e.target.value, true)}/></label>
                <label><SettingLabel label={t('modelSettings.topP')} help={<ModelSettingsHelp title={t('modelSettings.topP')} description={t('modelSettings.topPTooltip')}/>}/><input className="model-settings-input" type="number" min="0" max="1" step="any" value={numberInputs.top_p ?? profile.top_p ?? ''} placeholder={t('modelSettings.modelDefault')} onChange={e => updateNumber('top_p', e.target.value, true)}/></label>
                ...
                <section className={`model-settings-advanced${advancedOpen ? ' is-open' : ''}`}>
                    ...
                    <div className="model-settings-advanced-panel" aria-hidden={!advancedOpen}><div className="model-settings-advanced-fields">
                        ...
                        {profile.capabilities?.seed && <label className="model-settings-option"><SettingLabel label={t('modelSettings.seed')} help={<ModelSettingsHelp title={t('modelSettings.seed')} description={t('modelSettings.seedTooltip')}/>} description={t('modelSettings.seedHelp')}/><input className="model-settings-input" type="number" min="0" max={MODEL_SETTING_INPUT_MAX.seed} value={numberInputs.seed ?? profile.seed ?? ''} placeholder={t('modelSettings.randomSeed')} onChange={event => updateNumber('seed', event.target.value, true)}/></label>}
                    </div></div>
                </section>

    ...
    return <section className="external-data-section">
        ...
        <div className="external-data-setup-card">
            ...
            {hasServiceKey && <div className="external-data-sync-panel">
                <div className="external-data-sync-row"><div className="external-data-sync-info"><strong>{t('externalData.syncAllData')}</strong><span className="external-data-sync-description">{t('externalData.syncAllDescription', {count: supportedEnabledSources.length})}</span><div className="external-data-sync-meta"><span>{t('externalData.lastUpdated', {date: lastSync})}</span><span>{t('externalData.documentCount', {count: formatter.format((syncStatus.document_count || 0) + (bizSupportSyncStatus.document_count || 0) + (kStartupSyncStatus.document_count || 0) + (housingSyncStatus.document_count || 0) + (lhComplexSyncStatus.document_count || 0) + (lhNoticeSyncStatus.document_count || 0))})}</span></div></div><div className="external-data-sync-actions"><button type="button" className="external-data-sync-refresh" onClick={() => void startAllSync()} disabled={isAllSyncing || supportedEnabledSources.length === 0} aria-label={t('externalData.updateAllData')}><RefreshCw className={isAllSyncing ? 'is-spinning' : ''} size={16}/></button></div></div>
                {isAllSyncing && <div className="external-data-overall-progress">{supportedEnabledSources.map(source => {
                    const status = source.id === 'gov24' ? syncStatus : source.id === 'bizSupport' ? bizSupportSyncStatus : source.id === 'kStartup' ? kStartupSyncStatus : source.id === 'housing' ? housingSyncStatus : source.id === 'lhLeaseComplex' ? lhComplexSyncStatus : lhNoticeSyncStatus;
                    const current = status.current || 0;
                    const total = status.total || 0;
                    const stageKey = source.id === 'gov24' ? (status.stage || 'list') : source.id === 'bizSupport' ? (status.stage || 'list') : source.id === 'kStartup' ? (status.stage || 'startupAnnouncements') : source.id === 'housing' ? (status.stage || 'housingRental') : source.id === 'lhLeaseComplex' ? (status.stage || 'lhLeaseComplex') : (status.stage || 'lhLeaseNotice');
                    const stageNumbers = source.id === 'gov24' ? GOV24_SYNC_STAGE_NUMBERS : source.id === 'bizSupport' ? BIZ_SUPPORT_SYNC_STAGE_NUMBERS : source.id === 'kStartup' ? K_STARTUP_SYNC_STAGE_NUMBERS : source.id === 'housing' ? HOUSING_SYNC_STAGE_NUMBERS : LH_SYNC_STAGE_NUMBERS;
                    ...
                    const errorMessage = status.error_code === 'request_limit_exceeded'
                        ? t('externalData.requestLimitExceeded')
                        : t(source.id === 'housing' ? 'externalData.housing.syncFailed' : 'externalData.syncFailed');
                    return <div className={`external-data-service-progress is-${status.status}`} key={source.sourceId}>
                        <div className="external-data-service-progress-header"><strong className="external-data-sync-stage">{stageNumber} / 3</strong><strong>{t(`externalData.sources.${source.id}.name`)}</strong><span className="external-data-sync-count">{formatter.format(current)} / {total ? formatter.format(total) : '?'}</span></div>
                        {status.status === 'failed' && <div className="external-data-service-progress-error">{errorMessage}</div>}
                        <div className="external-data-sync-progress-track"><span style={{width: `${percent}%`}}/></div>
                    </div>;
                })}</div>}
        ...
        {country === 'KR' && <div className="external-data-source-list"><div className="external-data-section-title"><span>{t('externalData.kr.title')}</span><span>{t('externalData.sourceCount', {count: DATA_SOURCES.length})}</span></div>
            {DATA_SOURCES.map(source => {
                ...
                const startSourceSync = source.id === 'bizSupport' ? startBizSupportSync : source.id === 'kStartup' ? startKStartupSync : source.id === 'housing' ? startHousingSync : source.id === 'lhLeaseComplex' ? () => startLhSync('kr.lh_lease_complex') : source.id === 'lhLeaseNotice' ? () => startLhSync('kr.lh_lease_notice') : startSync;

...
function MailPanel({accountId, selectedMessageId, onAttachFilesToChat}: {
    ...
    const applySignatureTemplate = (templateId: string) => {
        ...
        const templates: Record<string, string> = {
            business: `<table><tbody><tr><td style="width: 132px; padding: 0 18px 0 0; vertical-align: top; border: 0;"><span data-memo-image data-width="112"><img src="${signatureProfileCreative}" alt="${name}" style="width: 112px; height: 112px; object-fit: cover; border-radius: 999px; display: block;"></span></td><td style="padding: 2px 0; vertical-align: top; border: 0;"><p><strong><span style="color: #cc785c">${name}</span></strong><br>${role}</p><p>${email}<br>${phone}<br><span style="color: #cc785c">${website}</span></p></td></tr></tbody></table>`,
            compact: `<p><strong>${name}</strong> · ${role}<br>${email} · ${phone}</p>`,
            modern: `<table><tbody><tr><td style="width: 108px; padding: 0 16px 0 0; vertical-align: middle; border: 0;"><span data-memo-image data-width="88"><img src="${signatureProfileModern}" alt="${name}" style="width: 88px; height: 112px; object-fit: cover; border-radius: 10px; display: block;"></span></td><td style="padding: 0 0 0 16px; vertical-align: middle; border: 0; border-left: 3px solid #5b89b8;"><p><strong><span style="color: #5b89b8">${name}</span></strong><br>${role}</p><p>${email}<br>${phone}</p></td></tr></tbody></table>`,
            executive: `<table><tbody><tr><td style="width: 104px; padding: 0 16px 0 0; vertical-align: top; border: 0;"><span data-memo-image data-width="96"><img src="${signatureProfileExecutive}" alt="${name}" style="width: 96px; height: 96px; object-fit: cover; border-radius: 50%; display: block;"></span></td><td style="padding: 0; vertical-align: top; border: 0;"><p><strong><span style="color: #4c7a61">${name}</span></strong><br>${role}</p><p>${email}<br>${phone}</p></td></tr></tbody></table>`,

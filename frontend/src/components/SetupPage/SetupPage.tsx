                                    ...
                                    {!isSearchingHub && vyactHardware.system_memory.total_bytes > 0 && (
                                        <div className="vyact-memory-summary setup-memory-summary">
                                            <div className="vyact-memory-capacity">
                                                ...
                                                {vyactHardware.memory_mode !== 'unified' && vyactHardware.gpus.map(gpu => <span key={`${gpu.backend}-${gpu.name}`}>{gpu.name}{gpu.total_bytes ? <> · {t('main:modelSelector.vram')} <strong>{formatModelBytes(gpu.total_bytes)}</strong></> : <strong>{gpu.backend}</strong>}</span>)}
                                    ...
                                    {!isSearchingHub && huggingFaceModels.map(model => {
                                        ...
                                        >
                                            ...
                                            <div className="vyact-model-files">{selectableFiles.map(file => {
                                                ...
                                                return <button className={`${selectedModel === modelPath ? 'is-selected ' : ''}memory-${getModelMemoryTone(estimatedMemory, vyactHardware)}`} key={file} type="button" onClick={event => { event.stopPropagation(); selectHubModelFile(model, file); }}><span className="vyact-model-file-name">{model.runtime === 'mlx' && <span className="vyact-mtp-badge">{t('main:modelSelector.mlxRuntime')}</span>}{supportsMtp && <span className="vyact-mtp-badge">MTP</span>}{supportsDFlash2 && <span className="vyact-mtp-badge">DFlash2</span>}<span>{displayName}</span></span>{estimatedMemory > 0 && <small className="vyact-model-file-meta">{fileSize > 0 && <>{formatModelBytes(fileSize)} · </>}{t('main:modelSelector.estimatedMemory')} ≈ {formatModelBytes(estimatedMemory)}{quantization && <span className="vyact-mtp-badge">{quantization}</span>}</small>}<span className="vyact-model-file-status">{selectedModel === modelPath && <Check size={15}/>}</span></button>;
                                            })}</div>
                                        </article>;
                                    })}
                                    </div>

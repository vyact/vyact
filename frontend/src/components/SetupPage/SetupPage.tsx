                    ...
                    <>
                        {provider === 'vyact' && (
                            <section className="setup-connection-panel setup-vyact-panel">
                                ...
                                <ModelStorageLocation confirmChanges={false} onBusyChange={setStorageBusy}/>
                                <div className="setup-hub-browser vyact-model-results">
                                    ...
                                    {!isSearchingHub && huggingFaceModels.map(model => {
                                        ...
                                        return <article
                                            className={`vyact-model-card${onlyFile ? ' is-compact' : ''}`}
                                            key={`${model.runtime}-${model.id}`}
                                        >
                                            {!onlyFile && <div className="vyact-model-card-heading"><OverflowTooltipText text={model.id}/><span>{formatCompactDownloads(model.downloads)}</span></div>}
                                            <div className="vyact-model-files">{selectableFiles.map(file => {
                                                ...
                                                return <button className={`${selectedModel === modelPath ? 'is-selected ' : ''}memory-${getModelMemoryTone(fileSize, vyactHardware)}`} key={file} type="button" onClick={event => { event.stopPropagation(); selectHubModelFile(model, file); }}><span className="vyact-model-file-name">{model.runtime === 'mlx' && <span className="vyact-mtp-badge">{t('main:modelSelector.mlxOnly')}</span>}{supportsMtp && <span className="vyact-mtp-badge">MTP</span>}{supportsDFlash2 && <span className="vyact-mtp-badge">DFlash2</span>}<OverflowTooltipText text={displayName || model.id}/></span><small className="vyact-model-file-meta"><span className="vyact-model-publisher">@{model.id.split('/')[0]}</span>{fileSize > 0 && <> · {t('main:modelSelector.modelFileSize')} · {formatModelBytes(fileSize)}</>}</small><span className="vyact-model-file-status"><span className="vyact-model-file-status-top">{onlyFile && <span className="vyact-model-file-downloads">{formatCompactDownloads(model.downloads)}</span>}<span className="vyact-model-file-check" aria-hidden="true"><Check size={15}/></span></span><span className="vyact-model-file-stats">{quantization && <span className="vyact-mtp-badge">{quantization}</span>}</span></span></button>;
                                            })}</div>
                                        </article>;
                                    })}
                                    </div>

...
const KnowledgeCollectionsModal = ({isOpen, collections, onClose, onCreate, onUpdate, onDelete, onReorder}: Props) => {
    ...
    return <ModalOverlay className="system-prompt-overlay" onClose={onClose} closeOnBackdrop closeOnEscape={!editing && !browsing && !collectionToDelete}>
        ...
        {collectionToDelete && <ConfirmModal title={t('knowledgeCollections.deleteConfirmTitle', {name: collectionToDelete.name})} description={t('knowledgeCollections.deleteConfirmDescription')} options={[{label: t('knowledgeCollections.cancel'), value: 'cancel'}, {label: t('knowledgeCollections.delete'), value: 'delete', variant: 'danger'}]} actionLayout="horizontal" onClose={() => setCollectionToDelete(null)} onSelect={value => { if (value === 'delete') void onDelete(collectionToDelete.id).then(() => setCollectionToDelete(null)); else setCollectionToDelete(null);}}/>}
    </ModalOverlay>;
};

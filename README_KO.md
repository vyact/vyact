...
GGUF와 Apple Silicon용 MLX 모델을 앱 안에서 검색하고 비교할 수 있습니다. 감지된 시스템 RAM과 GPU VRAM, 모델 크기, 양자화 방식, 최대 컨텍스트와 하드웨어 기준 예상 메모리 사용량을 확인한 뒤 다운로드하면 Vyact가 모델과 로컬 런타임을 준비합니다.

호환되는 다중 GPU llama.cpp 환경에서는 자동 메모리 맞춤이 기본으로 적용되며, 필요하면 GPU별 분배를 직접 지정할 수 있습니다. 공개 모델은 API 키 없이 사용할 수 있고, 접근 승인이 필요한 모델은 권한이 있는 Hugging Face 계정의 키를 등록해 다운로드할 수 있습니다.
...
완료된 결과는 세 작업의 첫 토큰 시간과 256개 출력 토큰 기준 생성 시간을 결합한 속도 점수로 정렬됩니다. 이는 답변 품질이나 메모리 절감 효과가 아닌 속도 비교입니다. 원하는 결과의 **이 설정 사용**을 누른 뒤 **적용**하면 설정을 활성화할 수 있습니다. 테스트할 조합을 미리 확인해 전체 또는 일부만 실행할 수 있으며, 조합 간 컨텍스트·출력 한도·샘플링 설정은 동일하게 유지됩니다. 측정이 끝난 결과부터 저장하므로 테스트를 중단해도 완료된 측정값은 남습니다. 테스트 완료·취소·실패 후에는 이전 모델과 설정을 복원하고, 복원 오류가 있으면 알립니다. 같은 모델에서 새 테스트를 시작하면 이전 결과를 대체합니다.

<p align="center">
  <img src="assets/readme/feature-model-benchmark.png" alt="추천 설정과 작업별 시간 및 토큰 수를 보여주는 Vyact 모델 성능 테스트 결과" width="100%" />
</p>

### 문서를 검색 가능한 지식으로 만드는 RAG

문서를 한 번 색인하면 질문과 관련된 구간을 자동으로 찾아 AI 답변의 맥락으로 사용합니다. 문서·메모·색인한 이메일 스레드를 지식 컬렉션으로 묶고, 대화에서 컬렉션을 선택해 검색 범위를 해당 자료로 제한할 수 있습니다. 실제로 검색된 원문 구간을 확인하고 저장된 파일과 청크를 관리하거나 더 이상 필요하지 않은 데이터를 삭제할 수 있습니다.
...
목표 언어로 말하면 Vyact가 음성을 인식하고 자연스럽게 응답합니다. 개별 문장을 암기하는 데서 나아가 실제 대화 흐름 속에서 표현을 반복 연습할 수 있습니다.

<p align="center">
  <img src="assets/readme/feature-voice-chat.png" alt="Vyact 음성 대화와 외국어 회화 연습" width="100%" />
</p>
...
| 기능 | 활용 방법 |
| --- | --- |
| AI 대화와 스트리밍 응답 | 로컬 또는 외부 모델로 대화하고 생성되는 답변을 바로 확인합니다. |
| 파일 첨부·지식 컬렉션·RAG | 문서·메모·색인한 이메일을 묶고 질문에 맞는 자료를 검색합니다. |
| 답변 근거 확인 | 응답에 활용된 문서와 원문 구간을 확인합니다. |
| 로컬 모델 성능 테스트 | 내 하드웨어에서 설정별 시간과 토큰 수를 비교하고 원하는 설정을 적용합니다. |
| 리치 텍스트 메모 | 아이디어와 계획을 정리하고 대화 중 RAG로 다시 찾습니다. |
| Google·Microsoft 연동 | 계정을 전환하며 메일·클라우드 파일·일정을 대화 옆에서 사용합니다. |
| 로컬 OpenAI 호환 API 서버 | 활성 로컬 모델을 OpenClaw나 같은 네트워크의 다른 앱에서 사용합니다. |
| 음성 대화와 답변 읽기 | 외국어 회화를 연습하고 생성 중인 답변을 음성으로 듣습니다. |
| 크롬 확장과 넷플릭스 학습 | 웹페이지를 번역·분석하고 이중 자막과 맞춤 설명으로 언어를 학습합니다. |
| 브라우저 글쓰기 도우미 | 문법과 어조를 다듬고 수정 전후를 비교합니다. |
| MCP와 재사용 가능한 스킬 | 외부 도구를 연결하고 반복 작업에 필요한 지침을 관리합니다. |
| 다국어 인터페이스 | 한국어·영어·일본어·중국어·태국어·베트남어·스페인어·프랑스어를 지원합니다. |

### 프로젝트와 지식을 이어서 사용하기

- **프로젝트와 대화 기록:** 대화를 프로젝트별로 묶고 전용 작업 지침을 설정합니다. 대화 이름을 바꾸거나 내보내고, 작업을 다시 시작할 때 기존 흐름을 이어갈 수 있습니다.
- **파일과 지식 컬렉션:** 파일을 한 대화에 첨부하거나 장기 지식으로 색인합니다. 문서·메모·이메일을 컬렉션으로 묶어 작업에 필요한 자료만 검색할 수 있습니다.
- **메모와 할 일:** 아이디어, 계획, 결정 사항과 간단한 할 일을 정리하고 필요할 때 RAG로 다시 찾습니다.
- **AI 설정:** 로컬 llama.cpp·MLX, OpenAI, Gemini, Claude 또는 OpenAI 호환 엔드포인트를 선택하고 컨텍스트·출력·샘플링·임베딩·청킹 설정을 조절합니다.

### 업무 도구를 연결하고 작업하기

- **Gmail:** 메일 검색·읽기, 라벨 관리, 메일과 첨부파일의 대화 연결, AI 답장 초안 작성, 서명 관리와 연결 계정으로 보내기를 지원합니다.
- **Google Drive:** 파일 탐색·검색·업로드·다운로드·이름 변경·복사·공유를 지원하며, 파일을 대화나 지식베이스에 연결할 수 있습니다.
- **Google Calendar:** 현재 작업 화면에서 일정을 확인하고 생성·수정·삭제합니다.
- **Google 연결:** **설정 > Google**에서 OAuth 자격증명 JSON을 등록하고 계정을 연결합니다. 내장 도구가 OAuth 권한으로 Gmail·Drive·Calendar API를 직접 호출하므로 별도의 외부 MCP 서버가 필요하지 않습니다.
- **Microsoft 연결:** **설정 > Microsoft**에서 Microsoft Entra 앱의 Client ID를 등록하고 Outlook 메일, OneDrive 파일과 Microsoft 일정을 사용합니다. 설정 화면에서 사전 준비와 로그인 방법을 안내합니다.
- **계정 전환:** 연결된 Google·Microsoft 계정을 하나의 목록에서 전환합니다. **Cmd+Shift+G**(Mac) 또는 **Ctrl+Shift+G**(Windows/Linux)로 메일·파일·일정 패널을 열고 닫습니다.
- **MCP와 스킬:** **설정 > AI 도구**에서 파일시스템, GitHub 또는 로컬·원격 MCP 서버를 연결합니다. 반복 작업에 사용할 지침은 **설정 > 스킬**에서 관리합니다.

### 데이터와 모델을 직접 관리하기

- **로컬 AI:** llama.cpp, Apple Silicon용 MLX와 로컬 임베딩을 사용해 핵심 작업 맥락을 내 컴퓨터에서 처리할 수 있습니다.
- **제공자 선택:** 작업에 따라 로컬 모델, 외부 AI 제공자 또는 조직에서 운영하는 OpenAI 호환 서버를 선택합니다.
- **로컬 API 서버:** **설정 > API 서버**에서 네트워크 주소, 활성 모델 ID, OpenClaw 설정과 curl 예제를 복사합니다. 선택적 Bearer 토큰 인증은 Vyact 게이트웨이를 통하는 로컬·원격 요청에 동일하게 적용됩니다.
- **데이터 전송 범위:** 메일이나 클라우드 파일을 외부 AI와 함께 사용하면 해당 내용이 선택한 제공자로 전송될 수 있습니다. Vyact가 관리하는 로컬 모델을 사용하면 대화 맥락을 외부 AI 제공자에게 보내지 않습니다.
- **백업과 복원:** 대화, 문서, 파일, 메모, 프롬프트, 설정, 제공자 연결, 프로젝트와 단어장을 내보내고 복원합니다. 선택한 Google Drive 또는 OneDrive 계정에 클라우드 백업을 저장할 수 있으며, OAuth 토큰은 백업에 포함되지 않습니다.

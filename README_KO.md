...
| **Google 문서 도구** | AI에 Docs 문서 생성·텍스트 추가·찾아 바꾸기, Sheets 셀 읽기·수정, Slides 슬라이드 추가·텍스트 찾아 바꾸기, Forms 설문 생성·질문 추가·응답 조회를 요청하세요. |
...
**세부 관리 기능도 제공합니다.** 대화를 즐겨찾기에 추가하거나 내보내고, 답변의 토큰 사용량과 처리 속도를 확인하세요. 지식 컬렉션에 별도 지침을 지정하고 문서·메모·색인한 이메일을 담을 수 있습니다. 메일 첨부파일·서명·정형 문구·수신자 그룹을 관리하고, 클라우드 파일을 정리하거나 공유하며, 모델 다운로드 위치를 선택할 수 있습니다.
...
| **선택한 글 번역·단어·듣기** | 웹페이지에서 글을 선택하면 나타나는 도구 모음에서 **상세 번역**, **단어**, **듣기**, **바로 번역**을 사용하세요. 번역과 단어 뜻을 확인하거나 선택한 글을 음성으로 들을 수 있습니다. |
...
| **데스크톱 AI의 Chrome 작업** | Vyact 데스크톱 채팅에서 브라우저 작업을 요청하세요. 연결된 확장이 Chrome에서 페이지 읽기·클릭·입력을 실행합니다. 로그인 등 직접 해야 할 단계에서는 잠시 멈출 수 있습니다. |

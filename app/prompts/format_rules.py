...
provider_type(ollama/openai/gemini/claude)은 "API를 어떻게 호출하느냐"의 축이고,
포맷 선호(XML vs 마크다운)는 "모델이 어떻게 학습됐느냐"의 축이라 서로 다른 문제다.
Ollama는 같은 provider라도 내부에서 gemma → llama3.1 → qwen 등으로 모델이 바뀔 수 있으므로,
provider_type이 아니라 실제 모델명(provider_config["model"])을 기준으로 분기한다.

새 모델을 추가/교체했을 때 이 파일의 리스트만 수정하면 된다.
"""

# 모델명 접두어 매칭 (ollama 모델명은 "gemma4:e4b"처럼 태그가 붙으므로 startswith 사용).

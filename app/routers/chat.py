...
async def _get_selected_external_context(question: str, resource_ids: list[str]) -> tuple[list[dict], str]:
    ...
    if not selected_ids.intersection({GOV24_SOURCE_ID, BIZ_SUPPORT_SOURCE_ID, K_STARTUP_SOURCE_ID, WELFARE_SOURCE_ID}):
        return [], ""
    candidates = []
    if GOV24_SOURCE_ID in selected_ids:
        try:
            candidates.extend(await search_gov24_candidates(question))
    ...
    if WELFARE_SOURCE_ID in selected_ids:
        try:
            candidates.extend(await search_welfare_candidates(question))
        except Exception as exc:
            logger.warning("[external_data] welfare candidate search failed: %s", exc)
...
async def query(req: QueryRequest):
    ...
    if articles:
        ...
        external_selected = bool({GOV24_SOURCE_ID, BIZ_SUPPORT_SOURCE_ID, K_STARTUP_SOURCE_ID, WELFARE_SOURCE_ID}.intersection(req.external_resource_ids))
        # URL 크롤링

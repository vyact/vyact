...
# keeps ordinary chat at 16K while leaving room for a useful answer.
DEFAULT_OUTPUT_RESERVE_TOKENS = 4096


def clamp_context_limit(value: int | float | None) -> int:
    """Return a supported context upper bound, never below the 16K floor."""
    try:
...
def select_context_window(messages: list[dict], max_context: int | float | None,
    ...
    """Choose 16K, then double until the estimated request fits the configured cap."""

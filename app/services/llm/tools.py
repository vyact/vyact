...
async def build_tool_directive(tool_names: list[str]) -> str:
    ...
    if any(n.startswith("browser_") for n in tool_names):
        directive += (
            ...
            "사용자가 브라우저 사용, 웹 검색, 최신 정보 확인 또는 출처 확인을 명시적으로 요청했다면 "
            "일반 지식으로 대신 답하지 말고 적절한 browser_* 도구를 호출해라. "
            ...
            "검색 결과 페이지의 제목·스니펫·AI 개요는 원문 확인이 아니며 최종 근거로 사용하지 마라. "
            "사용자가 뉴스, 비교, 조사, 요약 또는 복수 출처 정리를 요청했다면 검색 결과의 관련 링크 중 "
            "서로 다른 신뢰할 수 있는 출처를 충분히 선정하고, 각 URL을 browser_open으로 직접 연 뒤 "
            "browser_read로 본문을 확인해라. 일반적인 복수 출처 조사에서는 가능하면 최소 3개의 원문을 확인하고, "
            "확인할 URL 여러 개를 이미 알고 있고 각 페이지 결과에 따라 다음 URL을 바꿀 필요가 없다면 "
            "browser_read_urls에 URL 목록을 한 번에 전달해 한 라운드에서 순차적으로 읽어라. 반대로 다음 행동이 "
            "현재 페이지 내용에 따라 달라지면 기존 browser_open과 browser_read를 한 단계씩 사용해라. "
            "접근할 수 없는 페이지는 건너뛰고 다른 결과를 확인해라. 검색 결과 스니펫만 읽은 상태에서 조사를 끝내지 마라. "
            "각 주장에는 실제로 원문을 읽은 출처만 연결하고, 원문에서 확인되지 않은 세부 내용을 보충하거나 추측하지 마라. "
            "비밀번호, 인증번호, 결제정보는 입력하지 말고 사용자가 브라우저에서 직접 처리하도록 요청해라. "
            "CAPTCHA, 로그인, 2단계 인증, 동의 화면처럼 사용자만 완료할 수 있는 단계가 나타나면 작업을 종료하지 말고 "
            "browser_wait_for_user를 호출해 tool loop를 일시정지해라. 사용자가 브라우저에서 직접 처리하고 "
            "'완료하고 계속'을 누르면 현재 페이지가 자동으로 다시 전달되므로, 그 결과를 확인한 뒤 원래 작업을 이어가라. "
            "비밀번호나 인증번호를 채팅으로 보내 달라고 요구하지 마라. 사용자가 '작업 중단'을 선택하면 즉시 중단 사실을 알려라. "
            "browser_* 도구의 성공 결과가 현재 대화에 존재하지 않으면 웹을 검색·탐색·확인했다고 절대 주장하지 마라. "
            "특히 검색 날짜, 기사 제목, 발표 내용, URL 또는 출처를 기억이나 추측으로 만들어내지 마라. "
            "도구를 호출하지 않았거나 호출이 실패했다면 검색 결과처럼 답하지 말고, 웹 확인을 수행하지 못했다는 사실을 "
            "명확히 밝혀라. 최종 답변에는 browser_read로 실제 확인한 URL만 근거로 제시해라. "
            "브라우저 도구가 Vyact Chrome 확장이 연결되지 않았다는 오류와 설치 링크를 반환하면, 링크를 변경하거나 "
            "숨기지 말고 사용자가 바로 클릭할 수 있는 Markdown 링크로 그대로 안내해라."
        )

...
async def _fetch_announcements(service_key: str, progress_callback) -> list[dict]:
    records: list[dict] = []
    page = 1
...
async def synchronize(service_key: str) -> None:
    async with _sync_lock:
        ...
        status = {"status": "running", "stage": "list", "current": 0, "total": 0, "started_at": started_at, "document_count": previous.get("document_count", 0), "last_successful_sync_at": previous.get("last_successful_sync_at")}
        ...
        try:
            ...
            records = await _fetch_announcements(service_key, update_progress)
            if not records:
            ...
            for item in records:
                ...
                if not document["external_id"] or not document["title"]:
                    continue
...
async def search_candidates(question: str, size: int = 8) -> list[dict]:
    ...
    try:
        ...
        result = await es.search(index=INDEX_NAME, size=size, query={"bool": {
            ...
            "must": [{"multi_match": {"query": question.strip(), "fields": ["title^6", "target^4", "agency^3", "category^2", "hashtags^2", "content_text"], "operator": "or", "minimum_should_match": "20%"}}],
        }})
        candidates = []

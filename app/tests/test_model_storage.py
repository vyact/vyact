        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(storage, 'INSTALL_DIR', root / 'app'), \
                ...
                async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                    ...
                    self.assertEqual(status['parent_directory'], str(destination.parent.resolve()))
                    self.assertEqual(status['default_directory'], str((root / 'app').resolve()))
                    reset_plan = await client.post('/api/vyact/model-storage/plan', json={'path': status['default_directory']})
                    self.assertEqual(reset_plan.status_code, 200)
                    self.assertEqual(reset_plan.json()['destination'], str(source.resolve()))
                    ...
                    restored = await client.post('/api/vyact/model-storage/move', json={'path': status['default_directory']})
                    self.assertEqual(restored.status_code, 200)
                    await routes._move_task
                    restored_status = (await client.get('/api/vyact/model-storage')).json()
                    self.assertTrue(restored_status['is_default'])
                    self.assertEqual(restored_status['parent_directory'], str(source.parent.resolve()))
                    self.assertEqual((source / 'owner/model.gguf').read_bytes(), b'gguf' * 1024)

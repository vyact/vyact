...
Vyact는 Ollama 기반 로컬 LLM을 중심으로 설계했습니다. 대화, 문서, 업무 문맥을 내 환경 안에 두고 로컬 모델을 실제 업무를 위한 워크스페이스로 활용하세요. 필요할 때는 호스팅 모델이나 직접 운영하는 OpenAI-compatible LLM 엔드포인트도 연결할 수 있습니다.
...
- **내 환경에 맞춘 AI 제어** — Ollama, OpenAI, Gemini, Claude 또는 커스텀 OpenAI-compatible LLM 중 선택하고, 컨텍스트·출력·샘플링·임베딩·청킹·모델 메모리 유지 시간을 조정할 수 있습니다.
...
- **Provider는 내가 선택** — 일상적이고 민감한 작업에는 로컬 모델을 쓰고, 필요할 때 OpenAI·Gemini·Claude 또는 조직이나 다른 서비스에서 운영하는 OpenAI-compatible 엔드포인트를 연결할 수 있습니다.
...
- **중요한 데이터는 백업** — 대화, 문서, 원본 파일, 메모, 프롬프트, 설정, Provider 연결, 프로젝트, 단어장을 내보내고 복원할 수 있으며 Google Drive 백업도 지원합니다.
...
| 플랫폼 | 필수 | 선택 |
| --- | --- | --- |
| macOS (Apple Silicon) | 없음 | **로컬 LLM**<br>• 로컬 모델 사용 시에만 [Ollama](https://ollama.com/download)<br><br>**Elasticsearch**<br>• 컨테이너형은 Docker Desktop, 설치형도 지원<br><br>**Kokoro TTS**<br>• [Homebrew](https://brew.sh/)로 `espeak-ng` 설치 |
| Windows | 없음 | **로컬 LLM**<br>• 로컬 모델 사용 시에만 [Ollama](https://ollama.com/download)<br><br>**Elasticsearch**<br>• 컨테이너형은 Docker Desktop, 설치형도 지원<br><br>**Kokoro TTS**<br>• [Chocolatey](https://chocolatey.org/) 또는 `winget`으로 `espeak-ng` 설치 |

Apple Silicon Mac과 Windows에서는 Elasticsearch를 Docker Desktop으로 실행하거나 설치형으로 구성할 수 있습니다. 나머지 항목도 해당 기능을 사용할 때만 필요합니다. 처음 실행하면 Vyact가 선택한 구성에 필요한 요소를 자동으로 준비합니다.

### 처음 5분, 이렇게 시작하세요

1. Vyact를 실행한 뒤 Provider와 모델을 선택합니다. 로컬 우선 환경이라면 Ollama부터 시작하고, 이미 운영 중인 LLM 엔드포인트가 있다면 커스텀 LLM을 추가하세요.
...
### 커스텀 LLM Provider 연결하기

Vyact는 OpenAI-compatible `/chat/completions` API를 구현한 서버에 연결할 수 있습니다. 최초 설정에서는 **커스텀 LLM**을 선택하세요. 설치 후에는 사이드바의 Provider 컨트롤에서 연결을 추가하거나 편집할 수 있습니다.

다음 연결 정보를 설정합니다.

- **연결 이름** — Vyact에 표시할 이름입니다.
- **Base URL** — `/chat/completions`를 제외한 API 루트입니다. 예: `http://localhost:11434/v1`
- **API Key** — 로컬 서버에서는 선택 사항이며, Bearer 인증을 사용하는 엔드포인트에서는 필요합니다.
- **Model ID** — API가 요구하는 정확한 모델 식별자입니다.
- **추가 Header** — 게이트웨이나 조직별 인증에 필요한 Header를 선택적으로 추가합니다.

이미 실행 중인 로컬 Ollama는 다음 값으로 연결할 수 있습니다.
...
커스텀 연결 설정은 Vyact 백업 및 복원에 포함됩니다. 스트리밍, Tool calling, 이미지 입력은 연결한 서버와 모델의 기능 및 OpenAI 호환 범위에 따라 달라집니다.

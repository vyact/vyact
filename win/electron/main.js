...
async function executeFloatingBrowserCommand(command, args = {}) {
    ...
    if (command === "navigate") { await contents.loadURL(normalizeBrowserUrl(args.url)); return getFloatingBrowserState(); }
    if (command === "status") return getFloatingBrowserState();
    if (command === "read") return contents.executeJavaScript(`(() => { const hidden = element => !element || !element.isConnected || element.closest('[aria-hidden="true"]') || !element.getClientRects().length; const root = document.querySelector('main, article, [role="main"]') || document.body; const text = (root?.innerText || '').replace(/\\n{3,}/g, '\\n\\n').trim().slice(0, 20000); const links = Array.from(document.querySelectorAll('a[href]')).filter(a => !hidden(a)).slice(0, 100).map(a => ({text:(a.innerText || a.getAttribute('aria-label') || '').trim().slice(0,200),url:a.href})); return {url:location.href,title:document.title,text,links}; })()`);
    if (command === "inspect") return contents.executeJavaScript(`(() => { let index=0; const elements=Array.from(document.querySelectorAll('a[href],button,input,textarea,select,[role="button"],[role="link"]')).filter(el => el.isConnected && el.getClientRects().length && !el.disabled).slice(0,150); return elements.map(el => { const id='vyact-'+(++index); el.setAttribute('data-vyact-browser-id',id); return {id,tag:el.tagName.toLowerCase(),role:el.getAttribute('role') || '',name:(el.getAttribute('aria-label') || el.innerText || el.getAttribute('placeholder') || el.getAttribute('name') || '').trim().slice(0,200),type:el.getAttribute('type') || '',href:el.href || ''}; }); })()`);
    if (command === "click") { const elementId=JSON.stringify(String(args.element_id || "")); return contents.executeJavaScript(`(() => { const el=document.querySelector('[data-vyact-browser-id="'+CSS.escape(${elementId})+'"]'); if(!el)return {ok:false,error:'element_not_found'}; el.scrollIntoView({block:'center'}); el.click(); return {ok:true}; })()`); }
    if (command === "type") { const elementId=JSON.stringify(String(args.element_id || "")); const value=JSON.stringify(String(args.text || "")); return contents.executeJavaScript(`(() => { const el=document.querySelector('[data-vyact-browser-id="'+CSS.escape(${elementId})+'"]'); if(!el)return {ok:false,error:'element_not_found'}; if(el.type==='password')return {ok:false,error:'password_input_blocked'}; el.focus(); const setter=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(el),'value')?.set; if(setter)setter.call(el,${value}); else el.value=${value}; el.dispatchEvent(new Event('input',{bubbles:true})); el.dispatchEvent(new Event('change',{bubbles:true})); return {ok:true}; })()`); }
    if (command === "scroll") { const amount=Math.max(-4000,Math.min(4000,Number(args.amount || 700))); return contents.executeJavaScript(`(() => { window.scrollBy({top:${amount},behavior:'smooth'}); return {ok:true,scrollY:window.scrollY}; })()`); }
    if (command === "screenshot") { const image=await contents.capturePage(); return {url:contents.getURL(),image_base64:image.toPNG().toString("base64")}; }
...
function startBrowserControlServer() {
    ...
    return new Promise((resolve, reject) => {
        browserControlServer = http.createServer((request, response) => {
            if (request.method !== "POST" || request.url !== "/command" || request.headers.authorization !== `Bearer ${BROWSER_CONTROL_TOKEN}`) { response.writeHead(403,{"Content-Type":"application/json"}); response.end(JSON.stringify({ok:false,error:"forbidden"})); return; }
            let body="";
            request.on("data",chunk => { if(body.length < 1024*1024) body += chunk; });
            request.on("end",async () => { try { const payload=JSON.parse(body || "{}"); const result=await executeFloatingBrowserCommand(payload.command,payload.args || {}); response.writeHead(200,{"Content-Type":"application/json"}); response.end(JSON.stringify({ok:true,result})); } catch(error) { response.writeHead(500,{"Content-Type":"application/json"}); response.end(JSON.stringify({ok:false,error:error.message})); } });
        });
        browserControlServer.once("error",reject);
        browserControlServer.listen(0,"127.0.0.1",() => { browserControlPort=browserControlServer.address().port; resolve(); });
    });
}

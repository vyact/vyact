...
| 機能 | できること |
| --- | --- |
| **AI チャット** | ローカル・クラウドのモデルと会話し、ファイルや対応する画像を添付して、過去の会話を振り返れます。 |
| **文書と知識管理** | ファイルについて質問し、根拠となる原文を確認できます。よく使う資料は知識コレクションにまとめられます。 |
| **メール・クラウドファイル・予定** | Gmail、Outlook、Drive、OneDrive、カレンダーを接続し、返信の下書き、チャットへのファイル添付、予定の管理を行えます。 |
| **コード作業とレビュー** | プロジェクトフォルダーを接続し、AI でファイルを検索・編集して検査を実行できます。変更を確認し、追跡された編集を元に戻せます。 |
| **ローカルモデルの選択** | GGUF・Apple Silicon 用 MLX モデルを探し、メモリ使用量の見積もりを比較して、自分のパソコンで対応設定の性能を測定できます。 |

<details>
<summary><strong>デスクトップの機能をもっと見る</strong></summary>
...
| **Google Docs・Sheets・Slides・Forms** | AI に Docs 文書の作成・テキスト追記・検索と置換、Sheets のセルの読み書き、Slides のスライド追加・テキストの検索と置換、Forms の作成・質問追加・回答取得を依頼できます。 |
...
**細かな管理にも対応：** 会話のお気に入り登録やエクスポート、各応答のトークン使用量と処理速度の確認ができます。知識コレクションに専用の指示を設定し、文書・メモ・索引化したメールをまとめられます。メールの添付、署名、定型文、宛先グループの管理、クラウドファイルの整理・共有、モデルの保存先の選択も可能です。
...
| 機能 | できること |
| --- | --- |
| **チャットとページへの質問** | サイドパネルで会話し、現在のページや選択した文章について質問したり、読んでいる内容を要約したりできます。 |
| **文章チェックと書き直し** | スペル・文法の修正案や文章全体の書き直しを確認し、変更前後を比較して適用する内容を選べます。 |
| **翻訳・単語・読み上げ** | ページ上の文章を選択すると表示されるツールバーで、**詳細翻訳**、**単語**、**聞く**、**翻訳**を選べます。詳しい訳や単語の意味を確認し、選択した文章を聞いたり、ページ上に訳を表示したりできます。 |
| **単語帳と保存した文** | 意味や発音を調べ、例文とともに単語を保存できます。役立つ文は訳や出典リンクと一緒に保存できます。 |
| **Netflix で学習** | 二重字幕で学び、台詞を再生・リピートできます。全字幕のスクリプトを読み、字幕や表現について質問できます。 |

<details>
<summary><strong>Chrome 拡張機能をもっと見る</strong></summary>
...
| **デスクトップチャットから Chrome を操作** | Vyact のデスクトップチャットでブラウザー作業を依頼します。接続された拡張機能が Chrome でページの読み取り・クリック・入力を実行します。ログインなど自分で行う手順では一時停止できます。 |
...
必要なメモリとストレージ容量は、モデルとコンテキスト設定によって異なります。ダウンロード前に、Vyact に表示されるダウンロードサイズと推定メモリ使用量を確認してください。モデルの実行環境、文書、検索インデックス用の空き容量も必要です。ローカルモデルの実行がパソコンの負担になる場合は、外部 AI プロバイダーを接続することもできます。
